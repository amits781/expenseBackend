...
public class ExpenseService {
   ...
   * .getAsJsonArray()) { if (expenseUserJson.getAsJsonObject().get("user_id").getAsLong() == id) {
   * log.debug("Expense details user record found"); return expenseUserJson; } }
   * log.debug("Expense record not found"); return null; }
   * 
   * private String findCurrentUserPaymentInfo(JsonElement expenseUserDetails, Long id) { for
   * (JsonElement expenseUserJson : expenseUserDetails.getAsJsonObject().get("repayments")
   * .getAsJsonArray()) { if (expenseUserJson.getAsJsonObject().get("from").getAsLong() == id) {
   * log.debug("Payment Received found"); return "received"; } } log.debug("Payment done"); return
   * "done"; }
   */

  public List<ExpenseDTO> parseExpenseList(String expensesJsonString) {
    Type userListType = new TypeToken<ArrayList<ExpenseDTO>>() {}.getType();
    JsonElement userExpensesJson =
        parser.parse(expensesJsonString).getAsJsonObject().get("expenses");
  ...
  /*
   * public void createExpenses(String expensesJsonString) throws ParseException { JsonElement
   * userExpensesJson = parser.parse(expensesJsonString).getAsJsonObject().get("expenses"); Long
   * myId = userService.getCurrentUserObject().getId(); SimpleDateFormat format = new
   ...
   * findCurrentUserEntry(expenseJson, myId); String paymentStatus = "expense"; boolean isDeleted =
   * expenseJson.getAsJsonObject().get("deleted_at").isJsonNull() ? false : true; if (isDeleted) {
   * long id = expenseJson.getAsJsonObject().get("id").getAsLong(); log.debug("Deleted : " +
   * isDeleted + " | " + id); if (dao.hasEntry(id)) { log.debug("Deleted from DB: " + isDeleted +
   * " | " + id); dao.deleteById(id); } } if
   * (expenseJson.getAsJsonObject().get("payment").getAsBoolean()) paymentStatus =
   * findCurrentUserPaymentInfo(expenseJson, myId); if (myShareEntry != null && !isDeleted &&
   * (paymentStatus.equalsIgnoreCase("done") || paymentStatus.equalsIgnoreCase("expense"))) {
   * saveExpense(createExpenseObject(expenseJson, myShareEntry, format, paymentStatus)); } } }
   * 
   * private Expense createExpenseObject(JsonElement expenseJson, JsonElement myShareEntry,
   * SimpleDateFormat format, String paymentStatus) throws ParseException { Expense expense = new
   * Expense(); expense.setId(expenseJson.getAsJsonObject().get("id").getAsLong()); String
   * dateString = expenseJson.getAsJsonObject().get("date").getAsString(); dateString =
   * dateString.substring(0, dateString.length() - 1); Date date = format.parse(dateString);
   * expense.setDate(date);
   * expense.setDescription(expenseJson.getAsJsonObject().get("description").getAsString());
   * expense.setTotalCost(expenseJson.getAsJsonObject().get("cost").getAsFloat());
   * expense.setMyPaidShare(myShareEntry.getAsJsonObject().get("paid_share").getAsFloat()); if
   * (paymentStatus.equalsIgnoreCase("done")) { expense.setMyOwedShare(0.0f); } else {
   * expense.setMyOwedShare(myShareEntry.getAsJsonObject().get("owed_share").getAsFloat()); } return
   * expense; }
   */
