...
public class ExpenseService {
	...
	private JsonElement findCurrentUserEntry(JsonElement expenseUserDetails, Long id) {
		for (JsonElement expenseUserJson : expenseUserDetails.getAsJsonObject().get("users").getAsJsonArray()) {
					if(expenseUserJson.getAsJsonObject().get("user_id").getAsLong() == id) {
						log.debug("Expense details user record found");
						return expenseUserJson;
					}
				}
		log.debug("Expense record not found");
		return null;
	}
	
	private String findCurrentUserPaymentInfo(JsonElement expenseUserDetails, Long id) {
		for (JsonElement expenseUserJson : expenseUserDetails.getAsJsonObject().get("repayments").getAsJsonArray()) {
					if(expenseUserJson.getAsJsonObject().get("from").getAsLong() == id) {
						log.debug("Payment Received found");
						return "received";
					}
				}
	...
	public void createExpenses(String expensesJsonString) throws ParseException {
		...
		for (JsonElement expenseJson : userExpensesJson.getAsJsonArray()) {
			JsonElement myShareEntry = findCurrentUserEntry(expenseJson,myId);
			String paymentStatus = "expense";
			boolean isDeleted = expenseJson.getAsJsonObject().get("deleted_at").isJsonNull()? false:true;
			if(isDeleted) {
				long id = expenseJson.getAsJsonObject().get("id").getAsLong();
			...
			if(expenseJson.getAsJsonObject().get("payment").getAsBoolean())
				paymentStatus = findCurrentUserPaymentInfo(expenseJson,myId);
			if( myShareEntry != null && !isDeleted && (paymentStatus.equalsIgnoreCase("done") || paymentStatus.equalsIgnoreCase("expense"))) {	
				saveExpense(createExpenseObject(expenseJson, myShareEntry, format, paymentStatus));
			}
		}
	}
	
	private Expense createExpenseObject (JsonElement expenseJson, JsonElement myShareEntry, SimpleDateFormat format , String paymentStatus ) throws ParseException {
		Expense expense = new Expense();
		expense.setId(expenseJson.getAsJsonObject().get("id").getAsLong());
		String dateString = expenseJson.getAsJsonObject().get("date").getAsString();
		...
		expense.setMyPaidShare(myShareEntry.getAsJsonObject().get("paid_share").getAsFloat());
		if(paymentStatus.equalsIgnoreCase("done")) {
			expense.setMyOwedShare(0.0f);
		} else {
			expense.setMyOwedShare(myShareEntry.getAsJsonObject().get("owed_share").getAsFloat());
		}
		return expense;
	}
